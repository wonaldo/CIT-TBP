...
def al4causal(nodes, data, bandwidth, adjacency):
    ...
    for i, j in itr.product(nodes, repeat=2):
        ...
        # Causality from parents
            ...
            if sep_set_for_indtest.any():
                ...
                p_value = HSICtest.UIndtest(data[1:, [node_to_col_idx[i]]].reshape(-1, 1), data[1:, [node_to_col_idx[j]]].reshape(-1, 1))[0]
            ...
            if p_value<threshold:
                ...
                sep_set_ij= [data[1:-1, [node_to_col_idx[k]]].reshape(-1, 1) for k in sep_set_i if k in node_to_col_idx]
                sep_set_ji= [data[1:-1, [node_to_col_idx[k]]].reshape(-1, 1) for k in sep_set_j if k in node_to_col_idx]

                # Delta CTE
                TEij = kde.new_te(data[1:-1, [node_to_col_idx[i]]], data[2:, [node_to_col_idx[j]]], sep_set_ij, bandwidth)
                TEji = kde.new_te(data[1:-1, [node_to_col_idx[j]]], data[2:, [node_to_col_idx[i]]], sep_set_ji, bandwidth)
                delta_te = TEij - TEji

                # Significance test
                low_band, high_band = block_bootstrap(data[1:-1, [node_to_col_idx[i]]], data[2:, [node_to_col_idx[j]]], sep_set_ij, sep_set_ji, bandwidth, block_size=10, n_bootstrap=5000, low_per=2.5, high_per=97.5)
